...
def write_XYSIZE_data(graphs,labels,sizes,LEN_SEQUENCE,NUM_SEQUENCE,index,max_num, filename):
    ...
    for key,graph in graphs.items():
        ...
        temp = []
        temp_time = np.zeros(NUM_SEQUENCE,1) #store time
        size_temp = len(graph)
        ...
        for walk in graph:
            ...
            walk_time = math.floor(walk[1] / 1800)
            temp_time[walk_time] = 1
            temp_dict = {}
            if not temp_dict.has_key(walk_time):
                temp_dict[walk_time] = []
        ...
        for key, values in temp_dict.items():
            for value in values:
                for w in value:
                    for i in len(w):
                        nx_G.add_edge(nodes_items.get(w[i]), nodes_items.get(w[i + 1]))
            temp_adja = nx.to_pandas_adjacency(nx_G)
            N = len(temp_adj)
            if N < max_num:
                col_padding = np.zeros(shape=(N, max_num - N))
                A_col_padding = np.column_stack((temp_adj_i, col_padding))
