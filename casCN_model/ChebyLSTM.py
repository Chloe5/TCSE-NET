...
class chebLSTMCell(nn.Module):
    ...
    # 这里forward有两个输入参数，input_tensor 是一个五维数据c
    # （t时间步,b输入batch_ize,c输出数据通道数--维度,h,w图像高乘宽）
    # hidden_state=None 默认刚输入hidden_state为空，等着后面给初始化
    def forward(self, input, graph, cur_state):
        h_cur, c_cur = cur_state

        x = self.chebgcn1(input, graph)
        h_cur = self.chebgcn2(h_cur,graph)

        combined = torch.cat([x, h_cur], dim=1)
          # concatenate along channel
        cc_i, cc_f, cc_o, cc_g = torch.split(combined, self.hidden_dim, dim=1) #将combined的第1维 划分为hidden_dim块
    ...
    def init_hidden(self, batch_size, image_size):
        ...
        return (torch.zeros(batch_size, self.hidden_dim, height, width, device=self.chebgcn1.weight.device),
                torch.zeros(batch_size, self.hidden_dim, height, width, device=self.chebgcn1.weight.device))
...
class chebLSTM(nn.Module):
    ...
    def forward(self, input_tensor, graph, hidden_state=None):
        ...
        # 取出图片的数据，供下面初始化使用
        b, _, h, w = (np.array(input_tensor)).shape()
...
class MODEL(nn.Module):
    ...
    def forward(self, input_tensor, graph, n_step,
        ...
        last_h = self.chebylstm(input_tensor, graph)
